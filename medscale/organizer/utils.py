...
from medscale.organizer.cfg_client import env_name, root_path, fs_version

logger = logging.getLogger('medscale')
...
class SSHManager(object):
    ...
    def _exec_python_cmd(self, command):
        ...
        command = f'source ~/.bashrc; conda activate {env_name}; ' \
                  f'cd ~/{root_path}/medscale; {command}'
        _, stdout, stderr = self.ssh.exec_command(command)
        stdout = stdout.read().decode('ascii').strip("\n")
        stderr = stderr.read().decode('ascii').strip("\n")
        return stdout, stderr
    ...
    def _check_conda(self):
        ...
        output, err = self._exec_cmd(f'conda activate {env_name}; '
                                     f'python -c "import medscale; '
                                     f'print(medscale.__version__)"')
        if err:
            logger.error(err)
    ...
    def _check_source(self):
        ...
        """
        fs_path = os.path.join(root_path, 'medscale', '.git')

...
class HAT_Net(nn.Module):
    def __init__(self, img_size=100, in_chans=3, num_classes=3, dims=[64, 128, 256, 512],
...
def call_vit(model_config, local_data): 
    if ( 'vit_small_patch16_224_in21k' in model_config.type):
        ...
        return visitionT
    elif ('deit_base_patch16_224' in model_config.type):
        visitionT = timm.create_model('deit_base_patch16_224', pretrained=True)
        return visitionT
    elif ('deit3_base_patch16_224' in model_config.type):
        visitionT = timm.create_model('deit3_base_patch16_224', pretrained=True)
        return visitionT
    elif ('maxvit_rmlp_pico_rw_256' in model_config.type):
        visitionT = timm.create_model('maxvit_rmlp_pico_rw_256', pretrained=True)
        return visitionT
    elif ('wide_resnet50_2' in model_config.type):
        visitionT = timm.create_model('wide_resnet50_2', pretrained=True)
        return visitionT
    elif ('swinv2_cr_small_ns_224' in model_config.type):
        visitionT = timm.create_model('swinv2_cr_small_ns_224', pretrained=True)
        return visitionT

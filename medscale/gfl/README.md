# medscale-GNN: Towards a Unified, Comprehensive and Efficient Package for Federated Graph Learning

medscale-GNN (FS-G) is a unified, comprehensive and efficient package for federated graph learning. We provide a hands-on tutorial here, while for more detailed tutorial, please refer to [FGL Tutorial](https://medscale.io/docs/graph/).
...
The installation of FS-G follows medscale, please refer to [Installation](https://github.com/alibaba/medscale#step-1-installation).

After installing the minimal version of medscale, you should install extra dependencies ([PyG](https://github.com/pyg-team/pytorch_geometric), rdkit, and nltk) for the application version of FGL, run:
...
Now, you have successfully installed the FGL version of medscale.
...
```bash
python medscale/main.py --cfg medscale/gfl/baseline/example.yaml
```
...
* Node-level tasks, please refer to `medscale/gfl/baseline/repro_exp/node_level/`:

  ```bash
  # Example of FedAvg
  cd medscale/gfl/baseline/repro_exp/node_level/
...
* Link-level tasks, please refer to `medscale/gfl/baseline/repro_exp/link_level/`:

  ```bash
  cd medscale/gfl/baseline/repro_exp/link_level/
...
* Graph-level tasks, please refer to `medscale/gfl/baseline/repro_exp/graph_level/`:

  ```bash
  cd medscale/gfl/baseline/repro_exp/graph_level/
...
* Load your data (write in `medscale/contrib/data/`):
...
* Build your model (write in `medscale/contrib/model/`):
  ...
  from medscale.register import register_model
...
- Run with following command to start:
  ...
  ```bash
  python medscale/main.py --cfg medscale/gfl/baseline/example.yaml data.type mycora model.type gnn_mygcn
  ```
...
```latex
@inproceedings{medscalegnn,
  title     = {medscale-GNN: Towards a Unified, Comprehensive and Efficient Package for Federated Graph Learning},

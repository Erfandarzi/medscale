...
do
    ...
    do
        ...
        do
            ...
            python medscale/main.py --cfg medscale/cl/baseline/${train_yaml} device ${cudaid} federate.save_to ${save_path} federate.total_round_num ${total_round_num} data.splitter_args \[\{\'alpha\'\:${lda_alpha}\}\] train.optimizer.lr ${lrs[$i]} train.local_update_steps ${local_updates[$j]} train.batch_or_epoch ${batch_or_epoch} seed $k>>out/${method_name}_on_Cifar4CL_lda${lda_alpha}_lr${lrs[$i]}_lus${local_updates[$j]}_rn${total_round_num}${batch_or_epoch}.log 2>&1
            linear_prob_yaml=fedcontrastlearning_linearprob_on_cifar10.yaml
             python medscale/main.py --cfg medscale/cl/baseline/${linear_prob_yaml} device ${cudaid} federate.restore_from ${save_path} data.splitter_args \[\{\'alpha\'\:${lda_alpha}\}\] seed $k>>out/${method_name}_on_Cifar4CL_lda${lda_alpha}_lr${lrs[$i]}_lus${local_updates[$j]}_rn${total_round_num}${batch_or_epoch}.log 2>&1
        done
    done
done

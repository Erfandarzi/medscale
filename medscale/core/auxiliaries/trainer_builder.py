...
from medscale.core.trainers import Trainer

logger = logging.getLogger(__name__)

try:
    from medscale.contrib.trainer import *
except ImportError as error:
    logger.warning(
        f'{error} in `medscale.contrib.trainer`, some modules are not '
        f'available.')
...
def get_trainer(model=None,
    ...
    Arguments:
        ...
        is an attacker
        monitor: an instance of ``medscale.core.monitors.Monitor`` to \
    ...
    if config.trainer.type == 'general':
        if config.backend == 'torch':
            ...
            from medscale.core.trainers import GeneralTFTrainer
        ...
        if config.trainer.type.lower() in ['cvtrainer']:
            dict_path = "medscale.cv.trainer.trainer"
        elif config.trainer.type.lower() in ['nlptrainer']:
            dict_path = "medscale.nlp.trainer.trainer"
        elif config.trainer.type.lower() in ['cltrainer', 'lptrainer']:
            dict_path = "medscale.cl.trainer.trainer"
        elif config.trainer.type.lower() in [
                'graphminibatch_trainer',
        ]:
            dict_path = "medscale.gfl.trainer.graphtrainer"
        elif config.trainer.type.lower() in [
                'linkfullbatch_trainer', 'linkminibatch_trainer'
        ]:
            dict_path = "medscale.gfl.trainer.linktrainer"
        elif config.trainer.type.lower() in [
                'nodefullbatch_trainer', 'nodeminibatch_trainer'
        ]:
            dict_path = "medscale.gfl.trainer.nodetrainer"
        elif config.trainer.type.lower() in [
                'flitplustrainer', 'flittrainer', 'fedvattrainer',
                'fedfocaltrainer'
        ]:
            dict_path = "medscale.gfl.flitplus.trainer"
        elif config.trainer.type.lower() in ['mftrainer']:
            dict_path = "medscale.mf.trainer.trainer"
        elif config.trainer.type.lower() in ['atc_trainer']:
            dict_path = "medscale.nlp.hetero_tasks.trainer"
        else:
            raise ValueError
        ...
        from medscale.vertical_fl.trainer.utils import \
    ...
    if 'backdoor' in config.attack.attack_method:
        from medscale.attack.trainer import wrap_benignTrainer
    ...
    if is_attacker:
        ...
        from medscale.attack.auxiliary.attack_trainer_builder \
    ...
    if config.fedprox.use:
        from medscale.core.trainers import wrap_fedprox_trainer

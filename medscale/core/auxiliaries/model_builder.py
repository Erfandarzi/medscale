...
try:
    from medscale.contrib.model import *
except ImportError as error:
    logger.warning(
        f'{error} in `medscale.contrib.model`, some modules are not '
        f'available.')
...
def get_model(model_config, local_data=None, backend='torch'):
    ...
    if model_config.type.lower() == 'lr':
        if backend == 'torch':
            from medscale.core.lr import LogisticRegression
            model = LogisticRegression(in_channels=input_shape[-1],
                                       class_num=model_config.out_channels)
        elif backend == 'tensorflow':
            from medscale.cross_backends import LogisticRegression
            model = LogisticRegression(in_channels=input_shape[-1],
                                       class_num=1,

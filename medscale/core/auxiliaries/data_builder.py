...
from medscale.core.data.utils import RegexInverseMap, load_dataset, \
    convert_data_mode
...
try:
    from medscale.contrib.data import *
except ImportError as error:
    logger.warning(
        f'{error} in `medscale.contrib.data`, some modules are not '
        f'available.')
...
def get_data(config, client_cfgs=None):
    ...
    # counterpart
    translator = getattr(import_module('medscale.core.data'),
